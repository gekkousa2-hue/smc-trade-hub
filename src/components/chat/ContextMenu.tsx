...
export const ContextMenu = memo(function ContextMenu({
  contextMenuMsgId, contextMenuPos, messages, currentUserId, onStartEditing, onDelete, onTogglePin, onReply,
}: Props) {
  return (
    <AnimatePresence>
      {contextMenuMsgId && (
        <motion.div
          ...
          className="fixed z-[100] glass border border-border/50 rounded-xl shadow-xl overflow-hidden min-w-[180px]"
          style={{
            left: Math.min(Math.max(contextMenuPos.x - 90, 8), window.innerWidth - 188),
            top: Math.max(contextMenuPos.y - 140, 8),
          }}
        ...
        >
          {(() => {
            ...
            const canEdit = isOwn && !msg.media_type;
            return (
              <>
