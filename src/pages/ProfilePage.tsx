...
import {
  LogOut, Settings, History, ChevronRight, TrendingUp, Shield, Edit2, X, Check, Camera, Loader2,
} from "lucide-react";
...
export default function ProfilePage() {
  ...
  const handleSaveName = async () => {
    ...
    else { setUsername(editName.trim()); setShowSettings(false); toast.success("Ism yangilandi!"); }
  };
    ...
    <div className="flex flex-col items-center px-4 py-8 pb-28 min-h-full">
      <motion.div variants={container} initial="hidden" animate="show" className="flex flex-col items-center gap-5 w-full max-w-md">
        ...
        <motion.div variants={item} className="text-center space-y-2">
          <h2 className="font-display text-2xl font-bold text-foreground tracking-tight">{username || "Foydalanuvchi"}</h2>
          <div className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-primary/15 to-primary/5 border border-primary/30 px-3.5 py-1 shadow-[0_2px_12px_-4px_hsl(45_93%_58%/0.4)]">
            ...
            <span className="text-xs font-bold text-primary font-mono tracking-wider">ELITE TRADER</span>
          </div>
        </motion.div>
        ...
        <motion.div variants={item} className="w-full grid grid-cols-2 gap-3">
          <div className="premium-card rounded-2xl p-3.5 text-center">
            <p className="font-mono text-[9px] text-muted-foreground uppercase tracking-[0.15em]">Email</p>
            <p className="text-xs text-foreground truncate mt-1 font-mono">{user?.email || "—"}</p>
          </div>
          <div className="premium-card rounded-2xl p-3.5 text-center">
            <p className="font-mono text-[9px] text-muted-foreground uppercase tracking-[0.15em]">A'zo bo'lgan</p>
        ...
        <motion.div variants={item} className="w-full space-y-2.5 mt-2">
          <button
            onClick={() => { setEditName(username); setShowSettings(true); }}
          ...
          >
            ...
            <span className="text-sm font-semibold text-destructive flex-1 text-left">Chiqish</span>
          </button>
        </motion.div>
      </motion.div>
      ...
      <Dialog open={showSettings} onOpenChange={setShowSettings}>
        <DialogContent className="bg-card border-border max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-foreground font-display">Profilni tahrirlash</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            <div>
              <label className="text-xs text-muted-foreground font-mono uppercase tracking-wider">Foydalanuvchi nomi</label>
              <Input value={editName} onChange={(e) => setEditName(e.target.value)} className="mt-1.5 bg-muted border-border" />
            </div>
            <div className="flex gap-2">
              <button onClick={() => setShowSettings(false)} className="flex-1 flex items-center justify-center gap-1.5 rounded-xl bg-muted px-4 py-2.5 text-sm text-muted-foreground transition-colors hover:bg-accent">
              ...
              <button onClick={handleSaveName} disabled={saving} className="flex-1 flex items-center justify-center gap-1.5 rounded-xl bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50">
                <Check className="h-4 w-4" /> {saving ? "Saqlanmoqda..." : "Saqlash"}
              </button>
            </div>
          </div>
        </DialogContent>
      ...
      <Dialog open={showHistory} onOpenChange={setShowHistory}>
        <DialogContent className="bg-card border-border max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-foreground font-display">Savdo tarixi</DialogTitle>
          </DialogHeader>
          <div className="space-y-2 mt-2 max-h-72 overflow-y-auto scrollbar-thin">
            {mockHistory.map((trade, i) => (
              <div key={i} className="flex items-center justify-between glass rounded-xl px-4 py-3">
                <div>
                  <p className="text-sm font-medium text-foreground">
                    {trade.pair}{" "}
                    <span className={trade.type === "BUY" ? "text-success text-xs" : "text-destructive text-xs"}>{trade.type}</span>
                  </p>
                  <p className="text-[10px] text-muted-foreground font-mono">{trade.time}</p>
                </div>
                <span className={`text-sm font-bold font-mono ${trade.pnl.startsWith("+") ? "text-success" : "text-destructive"}`}>{trade.pnl}</span>
              </div>
            ))}
          </div>
        </DialogContent>
